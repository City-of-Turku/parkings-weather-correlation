...
A dedicated validation period from February 7 to March 9, 2025 was used to evaluate the forecasting performance of the MLPRegression, Prophet, and LinearRegression models, excluding weather variables. The following observations summarize the validation outcomes:

- MLPRegression consistently demonstrated the best forecasting accuracy among all models.
- All models showed limited effectiveness in short-term forecasting, performing worse than a naive baseline using the previous week's data.
- Forecast accuracy improves over longer time horizons, indicating these models are better suited for trend detection and long-term planning rather than short-term predictions.
